...
def random_phrase():
    ...
    # noun = noun[random_index]
    # random.sample(nouns, 1)[0]

    return adj + ' ' + selected_noun
...
def addy_split(addy_series):
    ...
    for addy in addy_series:
        ...
        zip_list.append(zip)
        # add the lists as a new columns on the df
    df['city'] = city_list
    df['state'] = state_list
    df['zip'] = zip_list
...
def abbr_2_st(state_series, abbr_2_st=True):
    ...
    def state_replace(state_name):
        ...
        return reverse_state_dict.get(state_name, state_name)
...
full_state_names_columns = abbr_2_st(addy_states)

# print(abbr_2_st(full_state_names_columns, abbr_2_st=False))
...
print(split_dates(pd.Series(['01/13/2016', '02/13/2017', '03/14/2018', '04/15/2019' ])))